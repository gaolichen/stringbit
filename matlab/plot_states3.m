function plot_states3(bits, statenumber, points, maxX, filename)
    if nargin < 3
    ...
    for i = xStart : length(X)
        ...
        [states, vec] = lowest_energies(bits, n, statenumber, true);
        if i < xStart + 1
            for j = 1 : statenumber
                Y(i, j) = states(j, 1);
                Yi(i, j) = states(j, 2);
            ...
            for j = 1 : statenumber
                for k = 1 : statenumber
                    if size(vec{k}, 1) ~= 0
                        mat((j-1)*statenumber + k, 1) = mynorm(vecs{j}, vec{k});
                    else
...
function f = mynorm(a, b)
    ...
    if i <= n
        f = f + norm(a(i:end,1))^2;
    end
    
    if j <= n
        f = f + norm(b(j:end,1))^2;
    end
end
